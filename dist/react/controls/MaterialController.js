...
export const MaterialController = ({ materials, transitionDuration = 0, children, className, }) => {
    ...
    const applyMaterialToMesh = (child, config) => {
        const wireframe = child.getObjectByName(`${child.name}-wireframe`);
        let newMat;
        switch (config.type) {
            case 'textured':
            ...
            case 'wireframe':
                ...
                if (wireframe) {
                    ...
                    wireframe.material.color.set(config.lineColor ?? 0x000000);
                }
                break;
            case 'custom':
    ...
    return _jsx("div", { className: className, children: children(items) });
};
