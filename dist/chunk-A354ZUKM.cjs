...
var SceneOrchestrator = (_class = class _SceneOrchestrator extends _chunkEA3XQ4KJcjs.THREE.EventDispatcher {
  ...
  async setModel(entry, options) {
    ...
    const model = await _chunkHMHIWIBXcjs.GLTFLoader.load(entry, {
      ...
      onLoaded: (obj) => {
        ...
        _optionalChain([options, 'optionalAccess', _2 => _2.onLoaded, 'optionalCall', _3 => _3(obj, entry)]);
        console.info(`[Orchestrator] Active model: ${entry.id}`);
      },
      onProgress: (...prev) => _optionalChain([options, 'optionalAccess', _4 => _4.onProgress, 'optionalCall', _5 => _5(...prev)]),
      onError: (err) => {
        _optionalChain([options, 'optionalAccess', _6 => _6.onError, 'optionalCall', _7 => _7(err, entry.url)]);
  ...
  async setHDRI(entry, config = {}) {
    try {
      if (_optionalChain([this, 'access', _8 => _8.activeHDRI, 'optionalAccess', _9 => _9.name]) === entry.id) {
        ...
        entry,
        {
          onLoaded: (tex, loadedEntry) => {
            ...
            console.info(`[Orchestrator] HDRI activo: ${loadedEntry.id}`, {
              ...
              format: _optionalChain([tex, 'access', _10 => _10.userData, 'optionalAccess', _11 => _11.format]),
  ...
  dispose() {
    ...
    for (const plugin of this.plugins.values()) {
      _optionalChain([plugin, 'access', _12 => _12.dispose, 'optionalCall', _13 => _13()]);
    }
    this.plugins.clear();
    this.removeModel();
    this.clearHDRI();
    this.renderer.dispose();
    _optionalChain([this, 'access', _14 => _14.renderer, 'access', _15 => _15.forceContextLoss, 'optionalCall', _16 => _16()]);
    this.canvas.width = 1;
