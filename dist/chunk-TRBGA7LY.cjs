...
var AdvancedOrbitControlsPlugin = (_class = class {
  ...
  update(options) {
    this.controls.dampingFactor = _nullishCoalesce(options.dampingFactor, () => ( 0.05));
    this.controls.panSpeed = _nullishCoalesce(options.panSpeed, () => ( 1));
    this.controls.rotateSpeed = _nullishCoalesce(options.rotateSpeed, () => ( 1));
    this.controls.zoomSpeed = _nullishCoalesce(options.zoomSpeed, () => ( 1));
    this.controls.minDistance = _nullishCoalesce(options.minDistance, () => ( 0.1));
    this.controls.maxDistance = _nullishCoalesce(options.maxDistance, () => ( 1e3));
    this.controls.minPolarAngle = _nullishCoalesce(options.minPolarAngle, () => ( 0));
    this.controls.maxPolarAngle = _nullishCoalesce(options.maxPolarAngle, () => ( Math.PI));
    this.controls.enablePan = _nullishCoalesce(options.enablePan, () => ( true));
    this.controls.enableRotate = _nullishCoalesce(options.enableRotate, () => ( true));
    this.controls.enableZoom = _nullishCoalesce(options.enableZoom, () => ( true));
  }
}, _class);
