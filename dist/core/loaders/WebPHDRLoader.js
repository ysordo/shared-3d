    ...
    load(url, onLoad, onProgress, onError) {
        ...
        loader.load(url, (buffer) => {
            try {
                ...
                const texture = new THREE.DataTexture(result.data, result.width, result.height, THREE.RGBAFormat, result.type);
        ...
        }, onProgress, (error) => onError?.(error));
    ...
    parse(buffer) {
        ...
        for (let i = 0; i < height; i++) {
            for (let j = 0; j < width; j++) {
                ...
                const sun = new THREE.Color(1.0, 0.9, 0.7).multiplyScalar(Math.exp(-Math.pow(phi - Math.PI, 2) / 0.1) * Math.exp(-Math.pow(theta - Math.PI / 6, 2) / 0.2) * 1000);
