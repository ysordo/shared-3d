...
export const createQuadWireframe = (geometry) => {
    ...
    for (let i = 0; i < indices.length; i += 3) {
        ...
        (isQuad ? legs : edges).forEach(({ verts: [V1, V2] }) => {
            ...
            if (!edgeMap.has(key)) {
                edgeMap.set(key, 1);
                vertices.push(position.array[v1 * 3], position.array[v1 * 3 + 1], position.array[v1 * 3 + 2], position.array[v2 * 3], position.array[v2 * 3 + 1], position.array[v2 * 3 + 2]);
            }
        });
    }
...
//# sourceMappingURL=QuadWireframe.js.map