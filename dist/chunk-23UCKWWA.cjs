...
var SceneOrchestrator = (_class = class _SceneOrchestrator extends _chunkEA3XQ4KJcjs.THREE.EventDispatcher {
  ...
  async setHDRI(entry, config = {}) {
    try {
      if (_optionalChain([this, 'access', _2 => _2.activeHDRI, 'optionalAccess', _3 => _3.name]) === entry.id) {
        ...
        entry,
        {
          onLoaded: (tex, loadedEntry) => {
            ...
            console.info(`[Orchestrator] HDRI activo: ${loadedEntry.id}`, {
              ...
              format: _optionalChain([tex, 'access', _4 => _4.userData, 'optionalAccess', _5 => _5.format]),
  ...
  dispose() {
    ...
    for (const plugin of this.plugins.values()) {
      _optionalChain([plugin, 'access', _6 => _6.dispose, 'optionalCall', _7 => _7()]);
    }
    this.plugins.clear();
    this.removeModel();
    this.clearHDRI();
    this.renderer.dispose();
    _optionalChain([this, 'access', _8 => _8.renderer, 'access', _9 => _9.forceContextLoss, 'optionalCall', _10 => _10()]);
    this.canvas.width = 1;
