...
var SceneOrchestrator = (_class = class _SceneOrchestrator extends _chunkEA3XQ4KJcjs.THREE.EventDispatcher {
  ...
  async setModel(entry, options) {
    ...
    if ("obj" in entry && entry.obj instanceof _chunkEA3XQ4KJcjs.THREE.Group) {
      ...
      _optionalChain([options, 'optionalAccess', _ => _.onLoaded, 'optionalCall', _2 => _2(entry.obj, entry.manifest)]);
      return entry.obj;
    } else {
      console.info(`[Orchestrator] Change model \u2192 ${entry.id}`);
      const model = await _chunkNY5P5I4Scjs.GLTFLoader.load(entry, {
        draco: _optionalChain([options, 'optionalAccess', _3 => _3.draco]),
        onLoaded: (obj) => {
          ...
          _optionalChain([options, 'optionalAccess', _4 => _4.onLoaded, 'optionalCall', _5 => _5(obj, entry)]);
          console.info(`[Orchestrator] Active model: ${entry.id}`);
        },
        onProgress: (...prev) => _optionalChain([options, 'optionalAccess', _6 => _6.onProgress, 'optionalCall', _7 => _7(...prev)]),
        onError: (err) => {
          _optionalChain([options, 'optionalAccess', _8 => _8.onError, 'optionalCall', _9 => _9(err, entry.url)]);
  ...
  async setHDRI(entry, config = {}) {
    try {
      if (_optionalChain([this, 'access', _10 => _10.activeHDRI, 'optionalAccess', _11 => _11.name]) === entry.id) {
        ...
        entry,
        {
          onLoaded: (tex, loadedEntry) => {
            ...
            console.info(`[Orchestrator] HDRI activo: ${loadedEntry.id}`, {
              ...
              format: _optionalChain([tex, 'access', _12 => _12.userData, 'optionalAccess', _13 => _13.format]),
  ...
  dispose() {
    ...
    for (const plugin of this.plugins.values()) {
      _optionalChain([plugin, 'access', _14 => _14.dispose, 'optionalCall', _15 => _15()]);
    }
    this.plugins.clear();
    this.removeModel();
    this.clearHDRI();
    this.renderer.dispose();
    _optionalChain([this, 'access', _16 => _16.renderer, 'access', _17 => _17.forceContextLoss, 'optionalCall', _18 => _18()]);
    this.canvas.width = 1;
