...
var SceneOrchestrator = (_class = class _SceneOrchestrator extends _chunkEA3XQ4KJcjs.THREE.EventDispatcher {
  ...
  async setHDRI(entry, config = {}) {
    try {
      ...
      const texture = await _chunkUQDXYJLBcjs.HDRILoader.load(
        entry,
        {
          onLoaded: (tex, loadedEntry) => {
            ...
            console.info(`[Orchestrator] HDRI activo: ${loadedEntry.id}`, {
              ...
              format: _optionalChain([tex, 'access', _2 => _2.userData, 'optionalAccess', _3 => _3.format]),
              exposure: config.exposure || _chunkUQDXYJLBcjs.HDRILoader.getOptions().exposure
            });
  ...
  dispose() {
    ...
    _optionalChain([this, 'access', _6 => _6.renderer, 'access', _7 => _7.forceContextLoss, 'optionalCall', _8 => _8()]);
    this.canvas.width = 1;
