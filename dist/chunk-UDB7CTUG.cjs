...
var _chunkP6OCPOFScjs = require('./chunk-P6OCPOFS.cjs');
...
var SceneOrchestrator = (_class = class _SceneOrchestrator {
  ...
  async setModel(entry, options) {
    ...
    const model = await _chunk2N3CMCMJcjs.GLTFLoader.load(entry, {
  ...
  async setHDRI(entry) {
    ...
    const texture = await _chunkP6OCPOFScjs.HDRILoader.load(entry, {
      onLoaded: (tex) => {
        this.activeHDRI = tex;
