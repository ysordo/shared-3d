...
var DistanceDisplay = ({
  ...
  const orchestrator = _chunkWAZQGQ6Zcjs.useScene.call(void 0, );
  ...
  const updateLimits = _react2.default.useCallback(() => {
    ...
    const collisionPlugin = orchestrator.plugin.get(
      "AdvancedCameraCollision"
    );
    if (collisionPlugin) {
      calculatedMin = collisionPlugin.distanceThreshold + collisionPlugin.pushBackOffset;
    }
    const controls = orchestrator.plugin.get(
      "AdvancedOrbitControls"
    ) || orchestrator.plugin.get("OrbitControls");
    if (controls) {
