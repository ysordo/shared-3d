...
class HDRLoader extends DataTextureLoader {
     ...
     */
	public parse( buffer: ArrayBuffer ): TexData {
		...
		const
			...
			rgbe_format_error = 3,
			rgbe_memory_error = 4,
			rgbe_error = function ( rgbe_error_code: number, msg?: string ): never {
			...
			fgets = function ( buffer: Uint8Array & { pos: number }, lineLimit?: number, consume?: boolean ): string | false {
			...
			RGBE_ReadHeader = function ( buffer: Uint8Array & { pos: number } ): RGBEHeader {
				...
				let line: string | false = fgets( buffer ), match: RegExpMatchArray | null;
			...
			RGBE_ReadPixels_RLE = function ( buffer: Uint8Array, w: number, h: number ): Uint8Array {
		...
		const RGBEByteToRGBFloat = function ( sourceArray: Uint8Array, sourceOffset: number, destArray: Float32Array, destOffset: number ): void {
		...
		const RGBEByteToRGBHalf = function ( sourceArray: Uint8Array, sourceOffset: number, destArray: Uint16Array, destOffset: number ): void {
		...
		const byteArray = new Uint8Array( buffer ) as Uint8Array & { pos: number };
		byteArray.pos = 0;
		...
		let data: Float32Array | Uint16Array, type: TextureDataType;
	...
	public override load( 
		...
		function onLoadCallback( texture: DataTexture, texData: object ): void {
